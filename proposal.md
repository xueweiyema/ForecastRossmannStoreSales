...
- 对数据做清洗和整理：
可能遇到的问题有异常值的处理，缺失值的处理，将store的数据和train，test数据关联起来，还有一些是宽表转长表。

- 对数据做单个变量分析，多个变量分析，取出影响结果的高相关度的变量：可能会遇到如何取出相关度高的特征值的问题。

- 使用深度学习方法中的适当方法建模：具体选取什么样的深度神经算法和激活函数。

- 通过不断的调参来完善模型：如何找到最合适的超参。
...
- 我使用Kaggle建议的RMSPE来做为验证函数，该值越低代表差异性越小。
